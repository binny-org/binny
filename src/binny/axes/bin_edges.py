...
def log_edges(x_min: float, x_max: float, n_bins: int) -> np.ndarray:
    """Returns log-spaced bin edges between ``x_min`` and ``x_max``.
    ...
    Returns:
        Array of bin edges of shape (n_bins + 1,), log-spaced.
    """
...
def equal_number_edges(x: Any, weights: Any, n_bins: int) -> np.ndarray:
    ...
    Returns:
        Array of bin edges of shape (n_bins + 1,), such that each bin has equal
        integrated weight.
    """
...
def equal_information_edges(
    ...
    along the axis. The method applies to any 1D axis (redshift, ell, k, etc.)
    and is useful for binning data with non-uniform information content. It is
...
def geometric_edges_n(x_min: float, x_max: float, n_bins: int) -> np.ndarray:
    """Returns geometrically bin edges between ``x_min`` and ``x_max``.

    The bins are log-spaced and include both endpoints.
    Works for any 1D axis: redshift, ell, k, etc and is useful for logarithmic
    scales.
