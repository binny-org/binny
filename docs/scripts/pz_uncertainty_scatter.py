...
OUTFILE = OUTDIR / "pz_uncertainty_scatter.gif"
...
scatter_vals = np.linspace(0.03, 0.12, 16)
...
ax.set_title("Photo-z bins with increasing scatter")
ax.set_xlim(0.0, 2.0)
...
kernel_x_center = 1.62
kernel_halfwidth_min = 0.035
kernel_halfwidth_max = 0.11
...
def draw_frame(frame_idx):
    ...
    s = scatter_vals[frame_idx]
    text_main.set_text(rf"$s = {s:.3f}$")

    t = (s - scatter_vals.min()) / (scatter_vals.max() - scatter_vals.min())
    kernel_halfwidth = (1.0 - t) * kernel_halfwidth_min + t * kernel_halfwidth_max
